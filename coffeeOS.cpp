...
void setup() { //___________________________________________ SETUP
...
Serial.begin(115200);
  ...
  //xTaskCreatePinnedToCore(webServerCode, "webServer", 10000, NULL, 5, &Task1, 0);
  ...
  count = 0;
	while(WiFi.status() != WL_CONNECTED) {
	  ...
	  count++;
	  delay(500);
		if(millis() - lastMillis == 10000) ESP.restart();
	  lastMillis = millis();
	}
...
void loop() { //___________________________________________ LOOP
  ...
  switch(menuID) { //-------------------------------
	case 0: { //____________________________________ Hauptmenü (Getränkeauswahl) 
	  ...
	  while(menuID == 0) {
		  ...
		  if(index < 0) {encoder.setCount(0); index = 0;}
		  if(index > len) {encoder.setCount(len * 4); index = len;}
	...
	case 1: { //____________________________________ Verbindung mit Kaffeemaschine
	...
	case 2: { //____________________________________ Getränk wird zubereitet --> Bildschirmschoner
	...
	case 3: { //____________________________________ Menü --> Einstellungen
	  ...
	  while(menuID == 3) {
		  ...
		  if(prevIndex != index) {
			...
			obdWriteString(&obd, 0, centerOffset(header, 8), 0, header, FONT_8x8, 0, 0);
			  ...
			  for(uint8_t i = 0; i <= len; i++) {
				if(i == index) obdWriteString(&obd, 0, 0, i + 1, menu1[i], FONT_8x8, 1, 0);
				else obdWriteString(&obd, 0, 0, i + 1, menu1[i], FONT_8x8, 0, 0);
			  }
		  ...
		  if(digitalRead(ROT_BUTTON) == ROT_BUTTON_PRESS) {
			...
			for(uint8_t i = 0; i < 5; i++) {
			  ...
			  case 0: menuID = 0; //__________________ Hauptmenü
			  break;
				  ...
				  if (digitalRead(PN532_IRQ) == LOW) {
					...
					Serial.println(userIndex);
						...
						if(userIndex == -1) {
						  ...
						  balance = 0;
						  for(uint8_t i = 0; i < (sizeof(prices)/sizeof(prices[0])); i++) balance += user[userIndex].memory[i] * prices[i];
			  ...
			  case 2: { //____________________________ User-Reset
				...
				while(index == 2) {
				  ...
				  if(index1 < 0) {encoder.setCount(0); index1 = 0;}
				  if(index1 > userIndex) {encoder.setCount(elements * 4); index = elements;}
				  ...
				  if(prevIndex != index1) {
					...
					balance = 0;
					for(uint8_t i = 0; i < (sizeof(prices)/sizeof(prices[0])); i++) balance += user[index1].memory[i] * prices[i];
					sprintf(sTemp, "Offen: %4.2f Euro", balance);
			  ...
			  case 4: { //____________________________ Helligkeit
				...
				while(index == 3) {
				  ...
				  Serial.println(brightness);
				  if(brightness < 2) {encoder.setCount(4); brightness = 2;}
				  if(brightness > 255) {encoder.setCount(100 * 4); brightness = 255;}
